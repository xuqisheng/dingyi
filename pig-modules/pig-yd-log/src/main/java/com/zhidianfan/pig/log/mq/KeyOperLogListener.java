 ...
 */
@Component
public class KeyOperLogListener {

    private Logger log = LoggerFactory.getLogger(getClass());

    @Autowired
    private ILogKeyOperService logService;

    @RabbitHandler
    @RabbitListener(queues = QueueName.LOG_KEY_OPER)
    public void onMessage(Message message, Channel channel) {
        byte[] body = message.getBody();

        try {
            String logObj = new String(body, 0, body.length, "utf8");
            LogKeyOper logTask = JsonUtils.jsonStr2Obj(logObj, LogKeyOper.class);
            //日志入库
            logService.insert(logTask);
        } catch (Exception e) {
            log.info("日志入库失败：\n{}", e.getMessage());
        }
